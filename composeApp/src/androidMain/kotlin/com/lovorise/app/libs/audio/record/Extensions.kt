...
import com.lovorise.app.libs.audio.record.config.AudioEncoder
...
internal fun RecordConfig.getOutput(context:PlatformContext): String {
    val fileName = "${System.currentTimeMillis()}${outputFormat.extension}"
    return when (this.outputLocation) {
        OutputLocation.Cache -> "${context.cacheDir.absolutePath}/$fileName"
        OutputLocation.Internal -> "${context.filesDir}/$fileName"
