...
import coinui.composeapp.generated.resources.do_you_have_any_pets
...
@Composable
fun UpdatePetSheetContent(
    pet: EditProfileScreenState.Pet,
    isDarkMode:Boolean,
    onSave:()->Unit,
    updatePet:(EditProfileScreenState.Pet)->Unit
) {


    var isButtonEnabled by remember { mutableStateOf(false) }

    var isVisible by remember { mutableStateOf(pet.isVisible) }
    var selectedIndex by remember { mutableIntStateOf(pet.selectedIndex) }
    ...
    Column {
        Column(
            ...
            Icon(
                ...
                imageVector = vectorResource(Res.drawable.ic_pets),
            ...
            Text(
                ...
                text = stringResource(Res.string.do_you_have_any_pets),
            ...
            FlowRow(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp), verticalArrangement = Arrangement.spacedBy(10.dp)){
                pet.items.forEachIndexed { index, value ->
                    ElevatedChipItem(
                        text = value,
            ...
            ButtonWithText(
                ...
                onClick = {
                    if (isButtonEnabled) {
                        updatePet(pet.copy(selectedIndex = selectedIndex, isVisible = isVisible))
                        onSave()
                    }
                }
            )
