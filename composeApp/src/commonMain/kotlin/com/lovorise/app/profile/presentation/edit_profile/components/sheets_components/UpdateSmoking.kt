...
@Composable
fun UpdateSmokingSheetContent(
    smoking: EditProfileScreenState.Smoking,
    isDarkMode:Boolean,
    onSave:()->Unit,
    updateSmoking:(EditProfileScreenState.Smoking)->Unit
) {


    var isButtonEnabled by remember { mutableStateOf(false) }

    var isVisible by remember { mutableStateOf(smoking.isVisible) }
    var selectedIndex by remember { mutableIntStateOf(smoking.selectedIndex) }

    LaunchedEffect(selectedIndex){
        isButtonEnabled = selectedIndex >= 0 || smoking.isVisible != isVisible
    }
    ...
    Column {
        Column(
            ...
            FlowRow(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp), verticalArrangement = Arrangement.spacedBy(10.dp)){
                smoking.items.forEachIndexed { index, value ->
                    ElevatedChipItem(
                        text = value,
            ...
            ButtonWithText(
                ...
                onClick = {
                    if (isButtonEnabled) {
                        updateSmoking(smoking.copy(selectedIndex = selectedIndex, isVisible = isVisible))
                        onSave()
                    }
                }
            )
