package com.lovorise.app.accounts.presentation.signup.family_planning
...
class FamilyPlanningScreen : Screen{
    ...
    override fun Content() {
        ...
        LaunchedEffect(true){
            accountsViewModel.setSignupPage(SignupFlowPages.FAMILY_PLANNING_SCREEN,ctx)
        }

        FamilyPlanningScreenContent(
...
@Composable
fun FamilyPlanningScreenContent(
    isDarkMode:Boolean,
    goBack:()->Unit,
    onSkip:()->Unit,
    onNext:()->Unit,
    ...
    val res = stringArrayResource(Res.array.children_preferences)
        ...
        Column(
            ...
            FlowRow(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp), verticalArrangement = Arrangement.spacedBy(10.dp)){
                items.forEachIndexed { index, value ->
                    ElevatedChipItem(text = value, isSelected = selectedIndex == index, onClick = {
            ...
            ButtonWithText(
                ...
                onClick = {
                    if (isEnabled && selectedIndex >= 0) {
                        accountsViewModel.addFamilyPlan(items[selectedIndex])
                    }
                }
            )
