...
import coinui.composeapp.generated.resources.reset_email
...
import coinui.composeapp.generated.resources.your_email_has_been_reset
...
import com.lovorise.app.settings.presentation.screens.ManageAccountScreen
...
class VerifyNewEmail(private val email: String) : Screen {
    ...
    override fun Content() {
        ...
        LaunchedEffect(true){
            accountsViewModel.changeEmail(ChangeEmailRequest(3,accountsState.changeEmailToken!!,null,email,null),ctx)
        }


        VerifyNewEmailScreenContent(
            email = email,
...
@Composable
fun VerifyNewEmailScreenContent(email:String,isDarkMode:Boolean,verifyEmailScreenModel: VerifyEmailScreenModel,onBack:()->Unit,navigateToSignIn:()->Unit,navigateToManageAccount:()->Unit,accountsViewModel: AccountsViewModel,accountsState:AccountsApiCallState) {
    val keyboardController = LocalSoftwareKeyboardController.current
    val focusManager = LocalFocusManager.current
    val invalidCode = stringResource(Res.string.verification_code_invalid)
        ...
        Column(
            ...
            Column(
                ...
                ButtonWithText(
                    ...
                    onClick = {
                        if (isEnabled) {
                            accountsViewModel.changeEmail(ChangeEmailRequest(4,accountsState.changeEmailToken!!,null,null,otpValue),ctx,email)
                            canNavigate = true
                        }
                    }
                )
    ...
    if (showResetSuccessDialog){
        CustomDialogWithTextAndBodyAndActions(
            ...
            body = buildAnnotatedString { append(stringResource(Res.string.your_email_has_been_reset))},
            title = stringResource(Res.string.reset_email),
            onAction1 = {
                showResetSuccessDialog = false
                navigateToSignIn()
                accountsViewModel.resetSuccessState()
