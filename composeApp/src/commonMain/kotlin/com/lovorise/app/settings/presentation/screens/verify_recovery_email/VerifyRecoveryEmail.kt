package com.lovorise.app.settings.presentation.screens.verify_recovery_email
...
class VerifyRecoveryEmail(private val email: String?) : Screen {
    ...
    override fun Content() {
        ...
        LaunchedEffect(true){
            ...
            accountsViewModel.addVerifyRecoveryEmail(data = AddVerifyRecoveryEmailRequest(page = 1, email = email ?: "", code = null), ctx = ctx)
        }



        VerifyRecoveryEmailScreenContent(
            email = email,
...
@Composable
fun VerifyRecoveryEmailScreenContent(email:String?,isDarkMode:Boolean,verifyEmailScreenModel: VerifyEmailScreenModel,onBack:()->Unit,navigateToManageAccount:()->Unit,accountsViewModel: AccountsViewModel,accountsState:AccountsApiCallState) {
    ...
    LaunchedEffect(accountsState.success){
        if (canNavigate) {
            if (accountsState.success) {
                accountsViewModel.updateEmailVerificationState(true)
                navigateToManageAccount()
    ...
    Column(
        ...
        Column(
            ...
            Column(
                ...
                Text(
                    text = stringResource(Res.string.enter_code_sent_to_your_email,(email ?: accountsState.user?.email) ?: ""),
                    fontFamily = PoppinsFontFamily(),
                ...
                ButtonWithText(
                    ...
                    onClick = {
                        if (isEnabled) {
                            canNavigate = true
                            accountsViewModel.addVerifyRecoveryEmail(data = AddVerifyRecoveryEmailRequest(page = 2, code = otpValue, email = null), ctx = ctx)
                        }
                    }
                )
