...
class WhoCanShareMyOnlineStatusScreen : Screen {
    ...
    override fun Content() {
        ...
        WhoCanShareMyOnlineStatusScreenContent(
...
@Composable
fun WhoCanShareMyOnlineStatusScreenContent(isDarkMode: Boolean, goBack:()-> Unit,state: PrivacyAndSecurityState,screenModel: PrivacyAndSecurityScreenModel,onAddPerson:()-> Unit){
    ...
    Column(modifier = Modifier.background(if (isDarkMode) BASE_DARK else Color.White)) {
        ...
        Column(
            ...
            Text(
                ...
                text = stringResource(Res.string.who_can_see_my_online_status),
            ...
            PrivacyTextWithCheckbox(
                text = "Everybody",
                isChecked = state.lastSeenAndOnline.privacyOption == PrivacyOptions.EVERYBODY,
                onClick = {
                    screenModel.updateLastSeenPrivacy(state.lastSeenAndOnline.copy(privacyOption = PrivacyOptions.EVERYBODY))
                }
            )
            ...
            PrivacyTextWithCheckbox(
                text = "Likes & Matches",
                isChecked = state.lastSeenAndOnline.privacyOption == PrivacyOptions.LIKES_AND_MATCHES,
                onClick = {
                    screenModel.updateLastSeenPrivacy(state.lastSeenAndOnline.copy(privacyOption = PrivacyOptions.LIKES_AND_MATCHES))
                }
            )
            Spacer(Modifier.height(8.dp))
            ...
            PrivacyTextWithCheckbox(
                text = "Matches only",
                isChecked = state.lastSeenAndOnline.privacyOption == PrivacyOptions.MATCHES_ONLY,
                onClick = {
                    screenModel.updateLastSeenPrivacy(state.lastSeenAndOnline.copy(privacyOption = PrivacyOptions.MATCHES_ONLY))
                }
            )
            Spacer(Modifier.height(8.dp))
            ...
            PrivacyTextWithCheckbox(
                text = "Nobody",
                isChecked = state.lastSeenAndOnline.privacyOption == PrivacyOptions.NOBODY,
                onClick = {
                    screenModel.updateLastSeenPrivacy(state.lastSeenAndOnline.copy(privacyOption = PrivacyOptions.NOBODY))
                }
            )
